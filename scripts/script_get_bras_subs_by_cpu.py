...
SUBS_OID = 'snmpget -v2c -c %s %s .1.3.6.1.4.1.9.9.786.1.2.1.1.5.1.%s'
...
def get_cpu_index():
  ...
  for line in out:
    ...
    if reparse.search(line):
      cpu_idx = reparse.search(line).group(2)
...
def get_index(idx):
  ...
  out = subprocess.Popen(['snmpget', '-v2c', '-c', COMMUNITY, HOST, '.1.3.6.1.4.1.9.9.786.1.2.1.1.5.1.%s' % idx +'.1'], stdout=subprocess.PIPE).communicate()[0]
...
if __name__ == "__main__":
  ...
  if 'query' in tip:
    ...
    print ''' 
script_get_bras_subs_by_cpu.py 1.1.1.1 public query index
script_get_bras_subs_by_cpu.py 1.1.1.1 public get index 66
'''
